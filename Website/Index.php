    ...
    <main>
        ...
        <form action="Index.php" method="get">
            <label>
                <input class="suche" type="text" name="search" placeholder="Search" >
            </label>
            <label>
                <select name="sort">
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        // Hier fügen wir den Link ein, der zur Detailseite führt
        echo '<a class="book-details-link" href="book_detail.php?id=' . $row["id"] . '">';

        // und das Bild nur einmal eingefügt wird
        echo '<img src="Bilder/cover.jpg" alt="' . $row["foto"] . '" alt="' . $row["kurztitle"] . '">';

        echo '<h2>' . $row["kurztitle"] . '</h2>';

        echo '</a>'; //  zur Detailseite führt
